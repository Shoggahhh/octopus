...
if __name__ == "__main__":
    ...
    # rocket_socket.get_file_from_mailru(
    #     "https://cloud.mail.ru/public/1ZWJ/4ncMnVjVa", "Прайс"
    # )
    ...
    # terneo.get_file_from_mailru(
    #     "https://cloud.mail.ru/public/1ZWJ/4ncMnVjVa", "Прайс лист"
    # )
    ...
    camelion.find_massage_from_outlook(
        "info@e-s-t.ru", f'Остатки товаров на {date.today().strftime("%d.%m.%Y")}'
    )
    camelion.transfer_to_ftp()

    italline = Octopus("Italline")
    italline.get_file_from_yandex(
        "https://disk.yandex.ru/i/r6i-TCZfuqr2qw", file_name="складские"
    )
    italline.transfer_to_ftp()

    ultraflash = Octopus("Ultraflash")
    ultraflash.find_massage_from_outlook(
        "info@e-s-t.ru", f'Остатки товаров на {date.today().strftime("%d.%m.%Y")}'
    )
    ultraflash.transfer_to_ftp()

    newport_stock = Octopus("Newport_stocks")
    newport_stock.get_file_from_link(
        "http://178.177.4.229:8087/publ/Остатки NEWPORT.csv"
    )
    newport_stock.transfer_to_ftp()

    newport_price = Octopus("Newport_price")
    newport_price.get_file_from_link(
        "https://newport-light.ru/Down/price-newport-mrc.xlsx"
    )
    newport_price.transfer_to_ftp()

    simple_story = Octopus("Simple story")
    simple_story.get_file_from_yandex(
        "https://disk.yandex.ru/d/hDHeLMp3dW6TRw", file_name="SIMPLE STORY"
    )

    ovivo = Octopus("Ovivo")
    ovivo.get_file_from_yandex(
        url="https://disk.yandex.ru/d/_YU5JEZnN0oRZQ", file_name="Остатки"
    )
    ovivo.transfer_to_ftp()

    bylectrica = Octopus("Bylectrica")
    bylectrica.get_file_from_yandex(
        url="https://disk.yandex.ru/d/_YU5JEZnN0oRZQ", file_name="Остатки"
    )
    ...
    arti_lampadari.get_file_from_yandex(
        "https://disk.yandex.ru/d/sa2zN832OAzAUQ", "Прайс Arti", "xlsx"
    )
    arti_lampadari.transfer_to_ftp()

    dio_de_arte = Octopus("Dio D'Arte")
    dio_de_arte.get_file_from_yandex(
        "https://disk.yandex.ru/d/sa2zN832OAzAUQ", "Прайс Dio", "xlsx"
    )
    dio_de_arte.transfer_to_ftp()

    lucia_tucci = Octopus("Lucia Tucci")
    lucia_tucci.get_file_from_yandex(
        "https://disk.yandex.ru/d/sa2zN832OAzAUQ", "Прайс Lucia", "xlsx"
    )
    lucia_tucci.transfer_to_ftp()

    ambrella_light = Octopus("Ambrella light")
    ambrella_light.get_file_from_mailru(
        "https://cloud.mail.ru/public/qxqq/ooTLNR8pp", "Прайс"
    )
    ambrella_light.transfer_to_ftp()

    top_decor = Octopus("TopDecor")
    top_decor.get_file_from_yandex(
        "https://yadi.sk/d/39sZ6hOa3GrqD5", "Остатки ТопДекор", "xlsx"
    )
    top_decor.transfer_to_ftp()
